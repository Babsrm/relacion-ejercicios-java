...
public class LibreriaFunciones {
	...
	//parece la misma funcion de arriba pero esta es para numeros int. es la misma funcion, sí, pero java sabe diferenciar cuál de las dos utilizar cuando la invocamos por el tipo de dato
	...
	public static int devolverMenor (int vector[]) {
		int menor = vector[0];
		//int menor = vector[vector.length-1]; con este cogeríamos el último pero es más sencillo empezar por la primera posición
		for (int i = 0; i < vector.length; i++) {
			if (vector[i] < menor) {
				menor = vector[i];
			}
